 ...
 * 修改时间：2018/12/21
 */
public class CustomDatePickerUtil implements View.OnClickListener, PickerView.OnSelectListener {
    ...
    private TextView mTvDayUnit,mTvHourUnit, mTvMinuteUnit,mTvSecondUnit;
    ...
    private static final int SCROLL_UNIT_SECOND = 0b11;
     ...
     */
    public CustomDatePickerUtil(Context context, Callback callback, String beginDateStr, String endDateStr) {
        this(context, callback, DateFormatUtils.str2Long(beginDateStr, DateFormatUtils.YMD_HM),
                DateFormatUtils.str2Long(endDateStr, DateFormatUtils.YMD_HM));
    }
     ...
     */
    public CustomDatePickerUtil(Context context, Callback callback, long beginTimestamp, long endTimestamp) {
    ...
    private void initView() {
        ...
        mPickerDialog.setContentView(R.layout.dialog_date_picker_util);
        ...
        title = mPickerDialog.findViewById(R.id.tv_title);
    ...
    private void initData() {
        ...
        if (canSpanYear) {
            ...
            initDateUnits(mEndMonth, mEndDay, mEndHour, mEndMinute,MAX_MINUTE_UNIT);
    ...
    private void linkageSecondUnit(final boolean showAnim) {
        if ((mScrollUnits & SCROLL_UNIT_SECOND) == SCROLL_UNIT_SECOND) {
            ...
            } else if (selectedYear == mEndYear && selectedMonth == mEndMonth && selectedDay == mEndDay && selectedHour == mEndHour && selectedMinute == mBeginMinute) {
                minSecond = 0;
                maxSecond = mEndSecond;
            } else {
                minSecond = 0;
     ...
     * 设置日期控件是否显示时和分
     */
    public void setCanShowPreciseTime(int canShowPreciseTime) {
        if (!canShow()) return;

        if (canShowPreciseTime == DateFormatUtils.YMD_HMS) {
            initScrollUnit();
            ...
            mTvSecondUnit.setVisibility(View.VISIBLE);
        }else if (canShowPreciseTime == DateFormatUtils.YMD_HM) {
            initScrollUnit();
            mDpvHour.setVisibility(View.VISIBLE);
            mTvHourUnit.setVisibility(View.VISIBLE);
            mDpvMinute.setVisibility(View.VISIBLE);
        ...
        } else  if (canShowPreciseTime == DateFormatUtils.YMD) {
