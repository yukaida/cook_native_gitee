 ...
 * desc：
 */
public class FocusPresenter extends BasePresenter<BaseView.FocusView, FocusFragmentSub>{



    public FocusPresenter(BaseView.FocusView view, FocusFragmentSub activity) {
        super(view, activity);
    }


    public void loadInfo(String openid,boolean isRefresh){
        ObserverOnNextListener<FocusResponse,Throwable> listener = new ObserverOnNextListener<FocusResponse, Throwable>() {
            @Override
            public void onNext(FocusResponse foundHomeInfo) {
        ...
        if (isRefresh){
            ApiMethods.focuinfo(new MyObserver<FocusResponse>(getActivity().getSupportActivity(),listener),"123","related","json",openid );
        }else {
            ApiMethods.focuinfo(new ProgressObserver<FocusResponse>(getActivity().getSupportActivity(),listener),"123","related","json",openid );
        }
    }
...
//    {"action":"more","total":已有数据条数,"openid":用户身份认证,"types":"square-related","diary_id":已加载最后一条数据的id,"lang":"zh"}
